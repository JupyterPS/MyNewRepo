...
# Step 3: Upgrade pip to the latest version with permissions fix
RUN python -m pip install --upgrade pip --no-cache-dir

# Step 4: Install necessary Python packages with permissions fix
...
RUN python -m pip install --upgrade --no-deps --force-reinstall notebook --no-cache-dir
RUN python -m pip install --user numpy spotipy scipy matplotlib ipython jupyter pandas sympy nose --no-cache-dir

# Step 11: Build JupyterLab
RUN jupyter lab build --dev-build=False --minimize=False

# Step 12: Install JupyterLab Git and related extensions using pip
RUN python -m pip install jupyterlab-git jupyterlab_github --no-cache-dir

# Step 13: Install Jupyter themes and additional Python packages
RUN python -m pip install jupyterthemes numpy spotipy scipy matplotlib ipython jupyter pandas sympy nose ipywidgets --no-cache-dir
